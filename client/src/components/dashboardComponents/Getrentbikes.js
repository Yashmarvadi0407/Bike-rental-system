...
let bikeIdFromDashBoard;
  const deleteUser = async(id) => {
    ...
    console.log(id);
    return await fetch("/deleteRentBikeFromDashboard", {
      method: "POST",
      headers:{
          "Content-Type" : "application/json"
      },
      body: JSON.stringify({
        bikeIdFromDashBoard:id
      })
  })
  }
        ...
        {getBikes.map((getBikes) => 
            <tbody   key={getBikes._id} >
                <tr>
                    ...
                    <td ><button id={getBikes._id} onClick={()=>deleteUser(getBikes._id)} className="btn"><i className="fa fa-trash"></i></button></td>
                </tr> 
            </tbody>
